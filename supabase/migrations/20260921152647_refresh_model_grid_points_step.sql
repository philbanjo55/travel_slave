drop procedure if exists public.refresh_model_grid_points(uuid, integer, numeric, integer);

-- One round: harvest whatever has come back, then queue the next batch.
-- Each call is its own transaction, so pg_net dispatches between calls.
-- Returns progress; no-ops cheaply when there is nothing to do.
--
-- Repeatable by design. Self-healing in three ways:
--   * a NEW STOP has no cached row, so it gets probed
--   * a MOVED STOP no longer matches requested_lat/lng, so it is re-probed
--   * a NEW MODEL in weather_models is probed for every stop
...
begin
  ...
  -- Clear everything settled, successes and failures alike; failures simply
  -- become missing again and are retried on a later call.
  ...
  select n_harvested, n_queued,
    (select count(*) from weather_models m
       cross join stops s join days d on d.id = s.day_id
...
-- Drains itself, then no-ops. This is what makes new and moved stops get
-- picked up without anyone remembering to do it.
