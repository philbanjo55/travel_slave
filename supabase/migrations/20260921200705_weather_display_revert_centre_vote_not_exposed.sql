-- Reverts 20260921... weather_display_v6_centre_vote. The centre vote is still
-- being evaluated, so the contract the app reads goes back to exactly what it
-- was: same keys, same version number, no behaviour change on the phone.
-- public.weather_centre_scores() stays in place and is queryable directly for
-- comparison work; nothing reads it.
